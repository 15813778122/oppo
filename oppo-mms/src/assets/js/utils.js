const utils = {
  ...
  /**
   * 封装一个缓冲运动，咋们的缓冲算法里速度跟时间没有关系的，所以这里不需要传时间，时间就算根据距离自动得到的
   * @param ele       <DOMObject>  要运动的元素对象
   * @param attr      <string>     运动的属性名
   * @param end       <number>     运动的终点，单位px
   * @param fn        <function>   回调函数，在运动结束以后执行的函数
  ...
  move1: function (ele,attr,end,fn) {
    ...
    ele.timer = setInterval(function () {
      ...
      // speed就是当前这一步要往前走的距离
      // 如果运动是负方向，distance小于0，speed也小于0，剩下最后几步负零点几的时候如果向上取整直接得到0了，不能运动到终点了
      var speed = distance > 0 ? Math.ceil(distance / 10) : Math.floor(distance / 10)

      // 走完一步以后start得往前加上这一步要走的距离
      start += speed
      // 把更新之后的start赋值给属性
      ele.style[attr] = start + 'px'
      // 判断终点
      // 由于缓冲运动最后是一像素一像素运动的，所以他一定能刚好到终点
      if (start === end) {
        clearInterval(ele.timer)
        fn && fn()
      }
